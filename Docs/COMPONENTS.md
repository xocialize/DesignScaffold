...
Current release: **0.6.0** · `https://github.com/xocialize/DesignScaffold` (public, MIT).
...
| **Timeline T3** — in/out brackets, gap indicators, marquee select, row-height resize | AB-A-0031 | Scope confirmed 2026-08-26: the scaffold draws **that** a gap exists (the lane knows where clips are not); the consumer decorates it with its own affordance and click target. Awaiting a signal that T2 has settled under real use. |
...
<!-- Generated by Tools/scan-adopters.py — do not hand-edit. Release 0.6.0. -->

| Adopter | Project | Products used | Files | Pin | |
|---|---|---|---|---|---|
| **ML[X] Audio Studio** | ml(x) | `DesignScaffold` | 2 | 0.4.2 | ⚠️ 0.6.0 available |
| **ML[X] LTX Studio** | ml(x) | `DesignScaffold`, `DesignScaffoldLoading`, `DesignScaffoldStageStepper`, `DesignScaffoldTimeline` | 6 | 0.5.0 | ⚠️ 0.6.0 available |
| **ML[X] Media Optimizer** | ml(x) | `DesignScaffold` | 3 | 0.2.0 | ⚠️ 0.6.0 available |
| **mlx-engine-swift** | MLXEngine | `DesignScaffold` | 5 | 0.4.2 | ⚠️ 0.6.0 available |
